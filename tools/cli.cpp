...
static bool DecodeJxlProgressive(const uint8_t* jxl_data, size_t jxl_size,
                                  uint8_t** pixels, uint32_t* xsize, uint32_t* ysize,
                                  uint32_t* channels) {
        ...
        if (status == JXL_DEC_ERROR) {
            ...
            *channels = info.num_color_channels + (has_alpha ? 1 : 0);
            ...
            printf("Image size: %u x %u, %u channels (alpha: %s)\n", 
                   *xsize, *ysize, *channels, has_alpha ? "yes" : "no");
        }
...
static bool EncodeJxlProgressive(const uint8_t* pixels, uint32_t xsize, uint32_t ysize,
                                  uint32_t channels, uint8_t** jxl_data, size_t* jxl_size,
                                  float quality) {
    ...
    if (JXL_ENC_SUCCESS != JxlEncoderSetParallelRunner(enc.get(),
            JxlThreadParallelRunner, runner.get())) {
    ...
    JxlEncoderFrameSettingsSetOption(frame_settings, JXL_ENC_FRAME_SETTING_PROGRESSIVE_DC, 1);
    JxlEncoderFrameSettingsSetOption(frame_settings, JXL_ENC_FRAME_SETTING_PROGRESSIVE_AC, 1);
    JxlEncoderFrameSettingsSetOption(frame_settings, JXL_ENC_FRAME_SETTING_QPROGRESSIVE_AC, 1);
...
static bool PixelsToImageMagick(const uint8_t* pixels, uint32_t xsize, uint32_t ysize,
                                  const char* filename) {
    try {
...
static bool GetImageMagickSize(const char* filename, uint32_t* xsize, uint32_t* ysize) {
    try {
        Magick::Image image(filename);
        *xsize = image.columns();
        *ysize = image.rows();
...
static bool ResizeImageMagick(const char* filename, uint32_t target_xsize, uint32_t target_ysize,
                                uint8_t** out_pixels, uint32_t* out_xsize, uint32_t* out_ysize) {
    try {
        Magick::Image image(filename);
        ...
        *out_pixels = (uint8_t*)malloc(pixel_count);
        ...
        // No flip needed
        image.write(0, 0, *out_xsize, *out_ysize, "RGBA", Magick::CharPixel, *out_pixels);
...
static bool PixelsToStbImage(const uint8_t* pixels, uint32_t xsize, uint32_t ysize,
                              const char* filename) {
...
// Get image size without loading full pixels
static bool GetStbImageSize(const char* filename, uint32_t* xsize, uint32_t* ysize) {
    int width, height, channels;
...
static bool EncodeToJxl(const char* input, const char* output, float quality) {
...
#else
    fprintf(stderr, "No image loading support compiled in (need ImageMagick or stb_image)\n");
    return false;
#endif
    ...
    if (!EncodeJxlProgressive(pixels, xsize, ysize, channels, &jxl_data, &jxl_size, quality)) {
...
static bool EncodeAnimation(const char** inputs, int count, const char* output, float quality) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (!ResizeImageMagick(inputs[i], target_xsize, target_ysize, &pixels, &actual_xsize, &actual_ysize)) {
    ...
    // Encode as animation (for now, encode as single frame - full animation needs more work)
    uint8_t* jxl_data = NULL;
    size_t jxl_size = 0;
    
    printf("Encoding animation (%d frames) to JXL...\n", count);
    
    // Encode the first frame as a demo (full animation requires JXL animation API)
    if (!EncodeJxlProgressive(all_frames, target_xsize, target_ysize, 4, &jxl_data, &jxl_size, quality)) {
        fprintf(stderr, "Encoding failed\n");
        free(all_frames);
        return false;
    }
...
static bool DecodeFromJxl(const char* input, const char* output) {
    ...
    if (!DecodeJxlProgressive(jxl_data, jxl_size, &pixels, &xsize, &ysize, &channels)) {
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 1) {
        ...
        char* input = tinyfd_openFileDialog(
            "Select Image File",
            "",
            0,
            NULL,
        ...
        if (IsJxlFile(input)) {
            ...
            snprintf(output, sizeof(output), "%s", input);
            char* dot = strrchr(output, '.');
            if (dot) strcpy(dot, ".jxl");
            else strcat(output, ".jxl");
        }
